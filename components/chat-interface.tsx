...
import { Send, User, Bot, MoreHorizontal, Search, FileText, ChevronDown, Layers, SplitSquareVertical, BookOpen, Square } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full">
      {process.env.NODE_ENV === 'development' && (
        <div className="bg-gray-100 p-1.5 sm:p-2 text-xs text-gray-600 rounded mb-1.5 sm:mb-2">
          User ID: {userId}
        </div>
      )}
      <div className="flex flex-col min-w-0 gap-3 sm:gap-6 md:gap-8 flex-1 overflow-y-auto pt-1 sm:pt-2 md:pt-4 pb-2 sm:pb-4">
        {messages.length === 0 ? (
          ...
          messages.map((m) => (
            <div 
              ...
              data-role={m.role}
            >
              <div className="flex gap-1.5 sm:gap-3 md:gap-4 w-full group-data-[role=user]/message:justify-end">
                {m.role !== 'user' && (
                ...
                <div className={cn(
                  "flex flex-col gap-1.5 sm:gap-3 md:gap-4",
                  ...
                  <div className="flex flex-row gap-2 items-start">
                    <div 
                      className={cn(
                        m.role === 'user' 
                          ? "flex flex-col gap-1.5 sm:gap-3 rounded-2xl sm:rounded-3xl bg-[#EDEFF2] text-primary-foreground px-2.5 sm:px-3 py-1.5 sm:py-2 text-black break-words" 
                          : "flex flex-col gap-1.5 sm:gap-3 prose dark:prose-invert max-w-full overflow-hidden break-words"
                      )}
                    >
                      {m.role === 'user' ? (
                        ...
                        <>
                          ...
                          {m.parts.some(part => part.type === 'tool-invocation') && (
                            <div className="mb-2 sm:mb-3 md:mb-4 bg-muted/30 rounded-lg overflow-hidden border border-border/50 text-sm">
                              {m.parts.map(part => 
        ...
        {isLoading && (
          <div className="w-full mx-auto max-w-3xl px-2 sm:px-4 group/message">
        ...
        <div className="shrink-0 min-w-[16px] min-h-[16px] sm:min-w-[20px] sm:min-h-[20px] md:min-w-[24px] md:min-h-[24px]"></div>
      </div>
      
      <form onSubmit={handleSubmit} className="sticky bottom-0 flex mx-auto px-2 sm:px-4 bg-background/95 backdrop-blur-sm pb-1.5 sm:pb-3 md:pb-6 gap-1.5 sm:gap-2 w-full md:max-w-3xl">
        <div className="relative w-full flex flex-col gap-1.5 sm:gap-3 md:gap-4">
          <textarea
            ref={textareaRef}
            className="flex w-full border border-input px-2.5 sm:px-3 py-1.5 sm:py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 min-h-[38px] sm:min-h-[42px] max-h-[30vh] overflow-y-auto resize-none rounded-xl sm:rounded-2xl bg-muted pb-7 sm:pb-8 md:pb-10 dark:border-zinc-700"
            ...
            rows={1}
          />
          <div className="absolute bottom-0 right-0 p-1 sm:p-1.5 md:p-2 w-fit flex flex-row justify-end">
            <Button 
